...
const agent = new SolanaStripeAgent(
  stripeKey,
  process.env.SOLANA_RPC_URL || "https://mainnet.helius-rpc.com/?api-key=c55c146c-71ef-41b9-a574-cb08f359c00c",
  solanaKeypairPath,
  openaiApiKey
);
  ...
  try {
    ...
    switch (toolName) {
      ...
      case "create_hybrid_payment":
        result = await agent.createHybridPayment(
          parameters.product_name,
          parameters.description,
          parameters.price_usd,
          parameters.solana_address
        );
        break;
...
app.get('/status', (req, res) => {
  ...
  const solanaStatus = `connected to ${process.env.SOLANA_RPC_URL || "https://mainnet.helius-rpc.com/?api-key=c55c146c-71ef-41b9-a574-cb08f359c00c"}`;
